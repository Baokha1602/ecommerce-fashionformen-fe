...
import {
  ...
  updateRankThunk,
  deleteRankThunk,
} from '../store/ranks-thunk';
import { clearError } from '../store/ranks-slice';
import { RankFormModal } from '../components/RankFormModal';
import { RANK_NAME_COLOR, RANK_NAME_LABEL } from '../constants/ranks-constants';
...
const RankBadge: React.FC<{ rankName: string }> = ({ rankName }) => {
  const color = RANK_NAME_COLOR[rankName] ?? '#888';
  const label = RANK_NAME_LABEL[rankName] ?? rankName;
  return (
    <span
      style={{
        display: 'inline-flex',
...
const RanksPage: React.FC = () => {
  ...
  const { list, loading, submitting, error } = useAppSelector((state) => state.ranks);
  ...
  const handleOpenEdit = (record: RankResponse) => { setEditing(record); setModalOpen(true); };
  const handleCloseModal = () => { setModalOpen(false); setEditing(null); };

  const handleSubmit = async (values: RankUpsertRequest) => {
    if (editing?.id != null) {
      const result = await dispatch(updateRankThunk({ id: editing.id, body: values }));
      if (updateRankThunk.fulfilled.match(result)) {
        message.success('Cập nhật hạng thành viên thành công!');
        handleCloseModal();
      }
    } else {
      const result = await dispatch(createRankThunk(values));
      if (createRankThunk.fulfilled.match(result)) {
  ...
  const handleDelete = (id: number) => {
    modal.confirm({
      ...
      cancelText: 'Hủy',
      onOk: async () => {
        const result = await dispatch(deleteRankThunk(id));
        if (deleteRankThunk.fulfilled.match(result)) {
  ...
  const columns: ColumnsType<RankResponse> = [
    {
      title: 'ID',
      dataIndex: 'id',
      width: 60,
    ...
    },
    {
      ...
      dataIndex: 'rankName',
      width: 160,
      render: (rankName: string) => <RankBadge rankName={rankName} />,
    },
    {
    ...
    },
    {
      ...
      align: 'center',
      width: 100,
      render: (_: unknown, record: RankResponse) => (
        <Space size={4}>
          <Button
            type="text"
            size="small"
