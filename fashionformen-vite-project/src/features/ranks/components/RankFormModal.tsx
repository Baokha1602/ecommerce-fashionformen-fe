...
import { RANKS_FORM_FIELDS, RANK_NAME_LABEL } from '../constants/ranks-constants';
import { RankUpsertRequestRankNameEnum } from '../types/ranks-type';
...
interface RankFormModalProps {
  ...
  submitting: boolean;
  onSubmit: (values: RankUpsertRequest) => void;
  onClose: () => void;
}

const rankNameOptions = Object.values(RankUpsertRequestRankNameEnum).map((v) => ({
  value: v,
  label: RANK_NAME_LABEL[v] ?? v,
}));

export const RankFormModal: React.FC<RankFormModalProps> = ({
  open,
  editing,
  submitting,
  onSubmit,
  ...
  const [form] = Form.useForm<RankUpsertRequest>();

  useEffect(() => {
    if (open) {
      if (editing) {
        form.setFieldsValue({
          rankName: editing.rankName as any,
          point: editing.point,
          rankDiscount: editing.rankDiscount,
        });
      } else {
        form.resetFields();
      }
  ...
  return (
    ...
    >
      <Form form={form} layout="vertical" className="pt-2">
        ...
        >
          <Select options={rankNameOptions} placeholder="Chọn hạng" />
        </Form.Item>
