...
} from '../store/product_tags-thunk';
import { clearError } from '../store/product_tags-slice';
import { ProductTagFormModal } from '../components/ProductTagFormModal';
...
const ProductTagsPage: React.FC = () => {
  ...
  const { list, loading, submitting, error } = useAppSelector((state) => state.productTags);
  const { message, modal } = App.useApp();

  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState<ProductTagResponse | null>(null);

  useEffect(() => {
    dispatch(fetchAllProductTagsThunk());
  }, [dispatch]);
  ...
  const handleOpenEdit = (record: ProductTagResponse) => { setEditing(record); setModalOpen(true); };
  const handleCloseModal = () => { setModalOpen(false); setEditing(null); };

  const handleSubmit = async (values: ProductTagRequest) => {
    if (editing?.id != null) {
      const result = await dispatch(updateProductTagThunk({ id: editing.id, body: values }));
      if (updateProductTagThunk.fulfilled.match(result)) {
        message.success('Cập nhật hạng thành viên thành công!');
        handleCloseModal();
      }
    } else {
      const result = await dispatch(createProductTagThunk(values));
      if (createProductTagThunk.fulfilled.match(result)) {
  ...
  const handleDelete = (id: number) => {
    modal.confirm({
      ...
      cancelText: 'Hủy',
      onOk: async () => {
        const result = await dispatch(deleteProductTagThunk(id));
        if (deleteProductTagThunk.fulfilled.match(result)) {
  ...
  const columns: ColumnsType<ProductTagResponse> = [
    {
      title: 'ID',
      dataIndex: 'id',
      width: 60,
    ...
    {
      title: 'Hạng',
      dataIndex: 'productTagName',
      width: 160,
      render: (productTagName: string) => <ProductBadgeGeneric val={productTagName} />,
    },
    {
    ...
    },
    {
      ...
      align: 'center',
      width: 100,
      render: (_: unknown, record: ProductTagResponse) => (
        <Space size={4}>
          <Button
            type="text"
            size="small"
