...
} from '../store/product_images-thunk';
import { clearError } from '../store/product_images-slice';
import { ProductImageFormModal } from '../components/ProductImageFormModal';
...
const ProductImagesPage: React.FC = () => {
  ...
  const { list, loading, submitting, error } = useAppSelector((state) => state.productImages);
  const { message, modal } = App.useApp();

  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState<ProductImagesResponse | null>(null);

  useEffect(() => {
    dispatch(fetchAllProductImagesThunk());
  }, [dispatch]);
  ...
  const handleOpenEdit = (record: ProductImagesResponse) => { setEditing(record); setModalOpen(true); };
  const handleCloseModal = () => { setModalOpen(false); setEditing(null); };

  const handleSubmit = async (values: ProductImagesRequest) => {
    if (editing?.id != null) {
      const result = await dispatch(updateProductImageThunk({ id: editing.id, body: values }));
      if (updateProductImageThunk.fulfilled.match(result)) {
        message.success('Cập nhật hạng thành viên thành công!');
        handleCloseModal();
      }
    } else {
      const result = await dispatch(createProductImageThunk(values));
      if (createProductImageThunk.fulfilled.match(result)) {
  ...
  const handleDelete = (id: number) => {
    modal.confirm({
      ...
      cancelText: 'Hủy',
      onOk: async () => {
        const result = await dispatch(deleteProductImageThunk(id));
        if (deleteProductImageThunk.fulfilled.match(result)) {
  ...
  const columns: ColumnsType<ProductImagesResponse> = [
    {
      title: 'ID',
      dataIndex: 'id',
      width: 60,
    ...
    },
    {
      ...
      width: 160,
      render: (productImageName: string) => <ProductBadgeGeneric val={productImageName} />,
    },
    {
    ...
    {
      title: 'Giảm giá',
      dataIndex: 'productImageDiscount',
      align: 'center',
      width: 110,
      render: (v: number) => (
        <Badge
    ...
    },
    {
      ...
      align: 'center',
      width: 100,
      render: (_: unknown, record: ProductImagesResponse) => (
        <Space size={4}>
          <Button
            type="text"
            size="small"
